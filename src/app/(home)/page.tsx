...
const HomePage = async () => {
	...
	const nextThreeDays: string[] = Array.from(
		{ length: daysPrevisualisationNumber },
		(_, index: number) => {
	...
	const meetings: Meeting[] = await MeetingService.getMeetings(
		{ startHour: new Date(formattedTodayDate) },
		Number(session.user.id),
	);
	...
	nextThreeDays.forEach((dateStr: string) => {
		comingMeetings.set(
			dateStr,
			meetings
				.filter((meeting: Meeting) => meeting.startHour.toISOString().startsWith(dateStr))
				.slice(0, 2),
		);
	});
	...
	return (
		<section className={styles.homePage}>
			<div className={styles.displayGrid}>
				...
				<KpiCard
					name={t('clients.client', { count: clients.length })}
