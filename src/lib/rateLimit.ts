...
export const getClientIp = (request: NextRequest | { headers: Headers | Record<string, string | string[] | undefined> }): string => {
...
export const enforceRateLimit = (
    ...
    const retryAfter: number = info.resetTime ? Math.max(0, Math.ceil((info.resetTime - Date.now()) / 1000)) : 60;
